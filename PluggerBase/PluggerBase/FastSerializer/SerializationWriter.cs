/******************************************************************\
 *      Class Name:     SerializationWriter
 ...
 *      -----------------------------------------------------------
 * The SerializationWriter produces a binary serialization from
...
namespace PluggerBase.FastSerializer
{
    ...
    public sealed class SerializationWriter : BinaryWriter
    {
            ...
            : base(stream)
        {
            ...
            Write(0);
            // Write placeholder for types
            Write(0);
            // Write placeholder for objects
            Write(0);

            objectTokens = new ArrayList();
            objectTokens.Add(null); // reserve 0 index for null
        ...
        public void WriteObject(object value)
        {
            ...
            else if (value is IFastSerializable)
            {
                writeTypeCode(SerializedType.FastSerializableType);
                IFastSerializable fsobject = (IFastSerializable)value;
                fsobject.Serialize(this);
            }
            else
            {
        ...
        public void WriteOptimized(object[] values1, object[] values2)
        {
            ...
            for (int i = 0; i < values2.Length; i++)
            {
                ...
                else if (value2 is IFastSerializable)
                {
                    writeTypeCode(SerializedType.PointerType);
                    WritePointer(value2);
                }
                else
                    WriteObject(value2);
        ...
        // Note: this needs to be WriteList, or will capture Write(string)
        public void WriteList<T>(IEnumerable<T> value)
        {
            if (value is List<T>)
                writeTypedArray(((List<T>)value).ToArray(), false);
            else
            {
                // copy to real list
                List<T> elts = new List<T>();
                foreach (T elt in value)
                    elts.Add(elt);
                writeTypedArray(elts.ToArray(), false);
            }
        }
        ...
        public int AppendTokenTables()
        {
            ...
            // skip null
            // only output size here-- not ready until now, and will be read first!
            write7bitEncodedSigned32BitValue(objectTokens.Count - 1);
            for (int i = 1; i < objectTokens.Count; i++)
            {
                if (objectTokens[i] is IFastSerializable)
                {
                    writeTypeCode(SerializedType.FastSerializableTypeType);
                    Type objectType = objectTokens[i].GetType();
                    WriteOptimized(objectType.AssemblyQualifiedName);
                }
                else
                    writeTypeCode(SerializedType.NullType);
            }
            ...
            return (int)(BaseStream.Position - initialPosition);
        }
        ...
        public void WriteTokenizedObject(object value, bool recreateFromType)
        {
            ...
            {
                // record class name in stringTokens
                Type type = value.GetType();
                stringLookup.Add(type.AssemblyQualifiedName);
        ...
        public void WritePointer(object value)
        {
            if (value == null)
                write7bitEncodedSigned32BitValue(0);
            else
                WriteTokenizedObject(value);
        }
